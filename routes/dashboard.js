...
// ==================================================
// DASHBOARD REALTIME (DENGAN PENCARIAN FLEKSIBEL)
// ==================================================
router.get('/', (req, res) => {
    ...
    const currentMonth = listBulan[date.getMonth()]; // Contoh: "Januari"
    const currentYear = date.getFullYear();          // Contoh: 2026

    console.log("--- DEBUG DASHBOARD ---");
    console.log("Mencari data yang mengandung kata:", currentMonth);

    // 2. QUERY DATABASE (PAKAI 'LIKE' AGAR LEBIH FLEKSIBEL)
    // Perhatikan: WHERE d.jadwal_PMC LIKE ?
    const sql = `
        SELECT 
            d.device_type,
            COALESCE(
    ...
    // Jadi '%Januari%' artinya: Cari apapun yang ada tulisan "Januari"-nya
    db.query(sql, [`%${currentMonth}%`], (err, results) => {
        ...
        console.log("Data ditemukan:", results.length); // Cek terminal ada berapa data
